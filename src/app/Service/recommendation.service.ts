...
export class RecommendationService {
  ...
  rankCourses<T extends RankableCourse>(
    ...
    if (matchers.length === 0 && !skill) return [];

    return (courses ?? [])
      .filter((c) => !exclude.has(String(c.id)))
      .map((c) => ({ c, score: this.scoreCourse(c, matchers, skill) }))
      .filter((s) => s.score > 0)
      .sort(
        (a, b) =>
          b.score - a.score ||
  ...
  private scoreCourse(c: RankableCourse, matchers: RegExp[], skill: string): number {
